...
@protocol DJSimplePingDelegate;

/*! Controls the IP address version used by SimplePing instances.
 */

typedef NS_ENUM(NSInteger, DJSimplePingAddressStyle) {
    DJSimplePingAddressStyleAny,          ///< Use the first IPv4 or IPv6 address found; the default.
    DJSimplePingAddressStyleICMPv4,       ///< Use the first IPv4 address found.
    DJSimplePingAddressStyleICMPv6        ///< Use the first IPv6 address found.
};
...
@interface DJSimplePing : NSObject
...
@property (nonatomic, weak, readwrite, nullable) id<DJSimplePingDelegate> delegate;

/*! Controls the IP address version used by the object.
 *  \details You should set this value before starting the object.
 */

@property (nonatomic, assign, readwrite) DJSimplePingAddressStyle addressStyle;
...
@protocol DJSimplePingDelegate <NSObject>
...
typedef struct DJICMPHeader DJICMPHeader;

__Check_Compile_Time(sizeof(DJICMPHeader) == 8);
__Check_Compile_Time(offsetof(DJICMPHeader, type) == 0);
__Check_Compile_Time(offsetof(DJICMPHeader, code) == 1);
__Check_Compile_Time(offsetof(DJICMPHeader, checksum) == 2);
__Check_Compile_Time(offsetof(DJICMPHeader, identifier) == 4);
__Check_Compile_Time(offsetof(DJICMPHeader, sequenceNumber) == 6);

enum {
    DJICMPv4TypeEchoRequest = 8,          ///< The ICMP `type` for a ping request; in this case `code` is always 0.
    DJICMPv4TypeEchoReply   = 0           ///< The ICMP `type` for a ping response; in this case `code` is always 0.
};

enum {
    DJICMPv6TypeEchoRequest = 128,        ///< The ICMP `type` for a ping request; in this case `code` is always 0.
    DJICMPv6TypeEchoReply   = 129         ///< The ICMP `type` for a ping response; in this case `code` is always 0.
};
