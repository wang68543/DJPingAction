...
typedef struct DJIPv4Header DJIPv4Header;

__Check_Compile_Time(sizeof(DJIPv4Header) == 20);
__Check_Compile_Time(offsetof(DJIPv4Header, versionAndHeaderLength) == 0);
__Check_Compile_Time(offsetof(DJIPv4Header, differentiatedServices) == 1);
__Check_Compile_Time(offsetof(DJIPv4Header, totalLength) == 2);
__Check_Compile_Time(offsetof(DJIPv4Header, identification) == 4);
__Check_Compile_Time(offsetof(DJIPv4Header, flagsAndFragmentOffset) == 6);
__Check_Compile_Time(offsetof(DJIPv4Header, timeToLive) == 8);
__Check_Compile_Time(offsetof(DJIPv4Header, protocol) == 9);
__Check_Compile_Time(offsetof(DJIPv4Header, headerChecksum) == 10);
__Check_Compile_Time(offsetof(DJIPv4Header, sourceAddress) == 12);
__Check_Compile_Time(offsetof(DJIPv4Header, destinationAddress) == 16);
...
static uint16_t dj_in_cksum(const void *buffer, size_t bufferLen) {
    // 
	size_t              bytesLeft;
    int32_t             sum;
...
- (NSData *)pingPacketWithType:(uint8_t)type payload:(NSData *)payload requiresChecksum:(BOOL)requiresChecksum {
    ...
    if (requiresChecksum) {
        ...
        icmpPtr->checksum = dj_in_cksum(packet.bytes, packet.length);
    }
...
- (void)sendPingWithData:(NSData *)data {
    ...
    switch (self.hostAddressFamily) {
        case AF_INET: {
            packet = [self pingPacketWithType:DJICMPv4TypeEchoRequest payload:payload requiresChecksum:YES];
        } break;
        case AF_INET6: {
            packet = [self pingPacketWithType:DJICMPv6TypeEchoRequest payload:payload requiresChecksum:NO];
        } break;
        default: {
            assert(NO);
        } break;
...
+ (NSUInteger)icmpHeaderOffsetInIPv4Packet:(NSData *)packet {
    ...
    NSUInteger                  result;
    const struct DJIPv4Header *   ipPtr;
    size_t                      ipHeaderLength;
    
    result = NSNotFound;
    if (packet.length >= (sizeof(DJIPv4Header) + sizeof(DJICMPHeader))) {
        ipPtr = (const DJIPv4Header *) packet.bytes;
        if ( ((ipPtr->versionAndHeaderLength & 0xF0) == 0x40) &&            // IPv4
             ( ipPtr->protocol == IPPROTO_ICMP ) ) {
            ipHeaderLength = (ipPtr->versionAndHeaderLength & 0x0F) * sizeof(uint32_t);
            if (packet.length >= (ipHeaderLength + sizeof(DJICMPHeader))) {
...
- (BOOL)validatePing4ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr {
    ...
    if (icmpHeaderOffset != NSNotFound) {
        icmpPtr = (struct DJICMPHeader *) (((uint8_t *) packet.mutableBytes) + icmpHeaderOffset);

        receivedChecksum   = icmpPtr->checksum;
        icmpPtr->checksum  = 0;
        calculatedChecksum = dj_in_cksum(icmpPtr, packet.length - icmpHeaderOffset);
        icmpPtr->checksum  = receivedChecksum;
        
        if (receivedChecksum == calculatedChecksum) {
            if ( (icmpPtr->type == DJICMPv4TypeEchoReply) && (icmpPtr->code == 0) ) {
...
- (BOOL)validatePing6ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr {
    ...
    if (packet.length >= sizeof(*icmpPtr)) {
        ...
        if ( (icmpPtr->type == DJICMPv6TypeEchoReply) && (icmpPtr->code == 0) ) {
            if ( OSSwapBigToHostInt16(icmpPtr->identifier) == self.identifier ) {
                uint16_t    sequenceNumber;
        ...
        for (NSData * address in addresses) {
            ...
            if ( address.length >= sizeof(struct sockaddr) ) {
                switch (addrPtr->sa_family) {
                    case AF_INET: {
                        if (self.addressStyle != DJSimplePingAddressStyleICMPv6) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
                    case AF_INET6: {
                        if (self.addressStyle != DJSimplePingAddressStyleICMPv4) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
